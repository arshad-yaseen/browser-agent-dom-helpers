      ...
      element(el) {
        ...
        for (const [name] of el.attributes) {
          ...
          if (name.startsWith('data-')) {
            if (!name.match(/^data-(testid|test|cy|qa|id|name|value|action|target)/)) {
        ...
        if (!isInteractive && !isStructural && !hasEventHandler) {
          const keepTags = new Set(['html', 'head', 'body', 'title', 'br', 'hr', 'wbr']);
...
const INTERACTIVE_ELEMENTS = new Set([
  ...
  'form', 'label', 'iframe', 'video', 'audio', 'details', 'summary',
  'dialog', 'menu', 'menuitem', 'fieldset', 'legend', 'datalist', 'output',
  'progress', 'meter', 'embed', 'object', 'param', 'track', 'map', 'area'
]);

// attributes essential for selection and understanding element purpose
const ESSENTIAL_ATTRIBUTES = new Set([
  'id', 'class', 'name', 'data-testid', 'data-test', 'data-cy', 'data-qa',
  ...
  'src', 'srcset', 'alt', 'title', 'controls', 'autoplay', 'loop', 'muted',

  'for', 'aria-label', 'aria-labelledby', 'aria-describedby', 'role',
...
const REMOVE_COMPLETELY = new Set([
  'style', 'script', 'noscript', 'meta', 'link', 'base', 'template'
]);
...
const STRUCTURAL_ELEMENTS = new Set([
  ...
  'aside', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'ul', 'ol', 'li',
  'dl', 'dt', 'dd', 'table', 'thead', 'tbody', 'tfoot', 'tr', 'td', 'th',
  'caption', 'col', 'colgroup', 'blockquote', 'pre', 'code', 'figure',
  'figcaption', 'strong', 'em', 'b', 'i', 'u', 's', 'small', 'mark',
  'del', 'ins', 'sub', 'sup', 'q', 'cite', 'abbr', 'time', 'address'
]);
